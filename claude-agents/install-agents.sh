...
PRIMARY_DESCRIPTION="Headless-browser swarm agent for background web automation (lookups, extractions, form-driven flows). Owns a private isolated context in the shared headless browser daemon, which auto-starts on first use — even after a crash, though the first agent attaching after one comes up without browser tools and reports the crash (relaunch that agent). Only one concurrent invocation per agent type — Claude Code shares identical inline MCP server configs across concurrent subagents, so run parallel agents on distinct types (browser-swarm-1 … browser-swarm-10, one type per concurrent agent, counted across the whole session). Subagents cannot see which types their siblings picked, so when browser work fans out through multiple concurrent orchestrator subagents, the parent must write a disjoint type range into each one's prompt — colliding types silently share one browser context whose tabs clobber each other. Sessions idle for 5 minutes are reaped — relaunch the agent when browser work resumes."
