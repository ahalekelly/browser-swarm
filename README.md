...
**The daemon is owned by launchd, not an MCP launcher.** Each start submits one uniquely labelled job containing the browser and its watchdog. The job removes itself when the browser ends, so launchd never converts a crash or deliberate idle stop into an automatic restart. This separation is load-bearing during crash recovery: the first post-crash MCP launcher deliberately exits nonzero, and its process supervisor may kill every descendant, but the restarted daemon remains alive for the relaunched agent.
