...
Never launch a browser yourself. Your tools attach an isolated context to a shared headless browser daemon that the launcher auto-starts. When startup fails, including after a crash, the session carries only `browser_swarm_error` in place of browser tools; read or call it, then stop and report its message to the orchestrator. If browser tools are missing even after ToolSearch or break on first use, the default is to stop and report rather than fetch pages with curl or plain HTTP, which usually defeats the point of a browser agent; flag any substitution prominently in your final message. Report that daemon logs are under __DIR__ (`chromium-browser.log` or `firefox-browser.log`). If working tools begin failing with a closed connection, stop and report that the daemon crashed; the next launch restarts it. If a first browser call fails with Playwright's `Timeout ... exceeded` while `createBrowserWithInfo` connects, its two-minute attach budget expired. The low-priority shared browser is busy, not crashed or wedged; report a busy host rather than stopping the daemon and cutting off other agents.
