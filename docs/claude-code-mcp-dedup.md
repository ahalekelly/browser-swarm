...
Make each concurrent subagent's server config unique. Each sibling agent definition in [`claude-agents/`](../claude-agents/) passes its own per-type launcher identifier (`1` … `5`), which makes the inline MCP configs byte-distinct and defeats the dedup. The launcher uses that identifier to allocate a private persistent output directory for every invocation, keeping MCP artifacts out of the working directory without embedding one fixed directory in the config.
...
- **The files must stay in sync, and their configs must stay different.** Normalizing the launcher identifier arguments to match silently reintroduces the shared browser.
